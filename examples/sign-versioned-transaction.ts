...
async function main() {
	...
	try {
		...
		const signedVersionedTx =
			await signer.signVersionedTransaction(versionedTx);
		console.log('   Versioned transaction signed successfully!');

		// Verify signature
		if (
			signedVersionedTx.signatures.length > 0 &&
			signedVersionedTx.signatures[0]
		) {
			const sigHex = Buffer.from(signedVersionedTx.signatures[0]).toString(
				'hex',
			);
		...
		console.log(
			`   Static Account Keys: ${signedVersionedTx.message.staticAccountKeys.length}`,
		);
		console.log(
			`   Instructions: ${signedVersionedTx.message.compiledInstructions.length}`,
		);
