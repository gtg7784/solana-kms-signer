...
async function main() {
	...
	try {
		...
		console.log(
			`   Total time: ${elapsed}ms (${Math.round(elapsed / messages.length)}ms per signature)`,
		);
		...
		for (let i = 0; i < recipients.length; i++) {
			...
			console.log(
				`   Transaction ${i + 1}: Transfer ${amount / LAMPORTS_PER_SOL} SOL to ${recipients[i].toString().substring(0, 8)}...`,
			);
		}
		...
		console.log(
			`   Total time: ${txElapsed}ms (${Math.round(txElapsed / transactions.length)}ms per transaction)`,
		);
		...
		const parallelTxs = recipients.map((recipient, _i) => {
			const tx = new Transaction();
			tx.recentBlockhash = blockhash;
			tx.feePayer = publicKey;
			tx.add(
		...
		console.log(
			`   Speedup: ${((txElapsed / parallelElapsed - 1) * 100).toFixed(1)}%`,
		);

		console.log('\n📊 Performance Summary:');
		console.log(`   Messages signed: ${signatures.length}`);
		console.log(
			`   Transactions signed: ${signedTransactions.length + parallelSigned.length}`,
		);
		console.log(
			`   Public key cached: Yes (1 KMS call instead of ${1 + signatures.length + signedTransactions.length + parallelSigned.length})`,
		);
		console.log(
			`   Total KMS Sign calls: ${signatures.length + signedTransactions.length + parallelSigned.length}`,
		);
		...
		console.log(
			`   Transaction IDs: ${txIds.map((txId) => txId.toString()).join(', ')}`,
		);
	} catch (error) {
